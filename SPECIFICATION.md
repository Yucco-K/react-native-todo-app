...
- **AIカテゴリ推測**: タイトル・内容からFirebase Cloud Functions + OpenAI APIで自動分類
  ...
  - レート制限: 1ユーザー1日10回まで
- **リマインド機能**:
  - **リマインド設定**: 3点メニューから日時を指定してリマインドを設定
  - **DateTimePickerUI**: カレンダーピッカーで日付と時刻を選択
  - **バリデーション**: 過去の日時は設定不可
  - **変更・削除**: 設定済みリマインドの変更や削除が可能
  - **自動プッシュ通知**: 設定時刻に到達すると自動でプッシュ通知を送信
    - グループTodo: 全メンバーに通知
    - 個人Todo: 本人のみに通知
  - **ログイン時チェック**: アプリ起動・フォアグラウンド復帰時に未読リマインドを自動確認
  - **通知モーダル**: 未読リマインドをモーダルで一覧表示
  - **通知済みフラグ**: 「わかった」をタップすると通知済みとして記録
  - **Firestoreフィールド**: `remindAt`（日時）、`remindNotified`（通知済みフラグ）
...
- **通知タイミング**: グループTodoの追加・削除・完了時、グループへの招待時
- **通知対象**:
